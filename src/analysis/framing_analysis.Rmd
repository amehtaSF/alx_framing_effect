...
df_proc <- df_raw %>% 
  ...
  mutate(keyIssue_cyber = ifelse(Key_issues_Norway == 4, 1, 0)) 
```
...
Separate models per vignette
```{r}
fit <- df_framing_l %>% 
  filter(vignette == "dp") %>% 
  left_join(df_inddiff) %>% 
  glm(choice ~ frame, family = "binomial", .)
summary(fit)
logit2prob(coef(fit)[["frameloss"]][[1]])  # pr of risky in loss frame
df_framing_l %>% 
  filter(vignette == "dp") %>% 
  ggplot(aes(x = frame, y = choice)) + 
  stat_summary()
```
...
fit <- df_framing_l %>% 
  filter(vignette == "dp") %>% 
  left_join(df_inddiff) %>% 
  glm(choice ~ paq*frame, family = "binomial", .)
summary(fit)
plot_model(fit, type = "int", terms="paq [all]")
...
df_framing_l %>% 
  filter(vignette == "dp") %>% 
  left_join(df_inddiff) %>% 
  ggplot(aes(x = paq, y = choice, color = frame)) + 
  geom_smooth()
```
...
df_framing_l %>% 
  ...
  geom_smooth()
```
> Significant interaction such that high alexithymia has reduced framing effect
...
fit <- df_framing_l %>% 
  mutate_at(vars(frame, vignette), as.factor) %>% 
  left_join(df_inddiff) %>% 
  glm(choice ~ frame*vignette*paq,family = "binomial", .)
summary(fit)
plot_model(fit, type = "int", terms="paq [all]")
```
...
Alexithymia predicting different key issues
```{r}
df_inddiff %>% 
  glm(keyIssue_digitization ~ paq, ., family = "binomial") %>% 
  summary
df_inddiff %>% 
  ggplot(aes(x = paq, y = keyIssue_digitization)) + 
  geom_smooth()
```

```{r}
df_inddiff %>% 
  glm(keyIssue_econ ~ paq, ., family = "binomial") %>% 
  summary
df_inddiff %>% 
  ggplot(aes(x = paq, y = keyIssue_econ)) + 
  geom_smooth()
```

```{r}
df_inddiff %>% 
  glm(keyIssue_employ ~ paq, ., family = "binomial") %>% 
  summary
df_inddiff %>% 
  ggplot(aes(x = paq, y = keyIssue_employ)) + 
  geom_smooth()
```

```{r}
df_inddiff %>% 
  glm(keyIssue_cyber ~ paq, ., family = "binomial") %>% 
  summary
df_inddiff %>% 
  ggplot(aes(x = paq, y = keyIssue_cyber)) + 
  geom_smooth()
```
